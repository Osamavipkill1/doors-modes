...
local ScreechCallCounter = 0 -- must live outside Screech.Run so BindToRenderStep names stay unique across overlapping calls
...
local FakeScreech2 = Model.entity
		...
		local v3 = FakeScreech2
			...
			ScreechCallCounter = ScreechCallCounter + 1;
			local u1 = ScreechCallCounter;
	...
	if game.Players.LocalPlayer.Character.Humanoid.Health <= 0 then
		pcall(function() SetDeathCause("Mimic Screech") end)
	end
	end
	p1.camShaker:ShakeOnce(6, 42, 0, 1);
			...
			u3 = true;
			local NameofModel = FakeScreech2.Name
			v3:Destroy();
    ...
    local ModelTest = LoadCustomInstance(configs.Model)
