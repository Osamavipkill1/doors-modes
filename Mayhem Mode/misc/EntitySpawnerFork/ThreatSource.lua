...
-- Clone the camera and set its parent to the workspace
local camara = cmr:Clone()
camara.Parent = game.Workspace

-- Move the original camera to the player's character
...
-- Tween the camera to look at the entityModel
local lookAtTween = TweenService:Create(camara, TweenInfo.new(1), {CFrame = CFrame.lookAt(camara.CFrame.Position, pos)})
lookAtTween:Play()
