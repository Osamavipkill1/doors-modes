...
local SelfModules = {
    DefaultConfig = LoadCachedModule("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/misc/EntitySpawnerFork/DefaultConfig.lua"),
    Functions = LoadCachedModule("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/misc/EntitySpawnerFork/Functions.lua"),
}
...
Spawner.createEntity = function(config)
    ...
    if typeof(entityModel) == "Instance" and entityModel.ClassName == "Model" then
        ...
        if entityModel.PrimaryPart then
            ...
            warn("[Mayhem/Source] createEntity: model has no PrimaryPart and no BasePart to fall back to. Model: " .. tostring(config.Model))
        end
    else
        warn("[Mayhem/Source] createEntity: LoadCustomInstance did not return a valid Model for: " .. tostring(config.Model) .. " — check the asset id / GitHub path.")
    end
end
...
Spawner.runEntity = function(entityTable)
    ...
    -- entityTable.Model was the original, never-parented instance forever after this
    -- point -- only the local clone above ever actually goes into the workspace. Any
    -- Debug hook that reads entityTable.Model expecting the live entity (Bleed's
    -- OnEntityStartMoving does) was seeing a permanently-unparented instance, so
    -- `entityTable.Model.Parent` was always nil. Point the table at the live clone.
    entityTable.Model = entityModel
    ...
    pcall(function()
        onDeathScreen = Plr.PlayerGui.MainUI.Death.HelpfulDialogue.Visible and not Plr.PlayerGui.MainUI.DeathPanelDead.Visible
    end)
    if CG:FindFirstChild("JumpscareGui") or onDeathScreen then
    ...
    entityConnections.movementTick = RS.Stepped:Connect(function()
        if entityModel.Parent and not entityModel:GetAttribute("NoAI") then
            ...
            if playerInSight then
                local _, onScreen = workspace.CurrentCamera:WorldToViewportPoint(entityModel.PrimaryPart.Position)
                if onScreen then
                ...
                then
                    task.spawn(function()
                        ...
                        task.spawn(function()
                            ...
                            local function isDeathPanelVisible()
                                local ok, visible = pcall(function()
                                    return Plr.PlayerGui.MainUI.DeathPanelDead.Visible
                                end)
                                return ok and visible
                            end
                            repeat
                                task.wait()
                            until isDeathPanelVisible() or (tick() - startTime) > 10
