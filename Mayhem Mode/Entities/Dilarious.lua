...
local dbgLabel
local dbgPinned, dbgToken = false, 0
do
    local ok, ui = pcall(function() return gethui() end)
    local parent = (ok and ui) or game.Players.LocalPlayer:WaitForChild("PlayerGui")
    local old = parent:FindFirstChild("DilariousDebug")
    if old then old:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "DilariousDebug"
    gui.ResetOnSpawn = false
    gui.DisplayOrder = 999
    gui.Parent = parent

    dbgLabel = Instance.new("TextLabel")
    dbgLabel.Size = UDim2.fromOffset(260, 34)
    dbgLabel.Position = UDim2.new(0.5, -130, 0, 70)
    dbgLabel.BackgroundColor3 = Color3.new(0, 0, 0)
    dbgLabel.BackgroundTransparency = 0.35
    dbgLabel.TextColor3 = Color3.new(1, 1, 1)
    dbgLabel.Font = Enum.Font.GothamBold
    dbgLabel.TextSize = 16
    dbgLabel.Visible = false
    dbgLabel.Parent = gui
end

-- hold = seconds to pin the message (touch/kill events). No hold = live readout,
-- which never overwrites a pinned message.
local function dbg(text, color, hold)
    if not hold and dbgPinned then return end
    dbgLabel.Text = text
    dbgLabel.TextColor3 = color or Color3.new(1, 1, 1)
...
entityTable.Debug.OnEntitySpawned = function()
    task.spawn(function()
        ...
        -- DEBUG: pin the distance the moment any part of the model physically touches you.
        for _, part in ipairs(dilarious:GetDescendants()) do
            if part:IsA("BasePart") then
                part.CanTouch = true
        ...
        proximityConn = RunService.Heartbeat:Connect(function()
            if not isAlive() then
                if not dbgPinned then dbgLabel.Visible = false end
            ...
            if not hrp or not dilarious.PrimaryPart then
                dbg("no HRP / PrimaryPart", Color3.fromRGB(255, 200, 60))
                return
            end

            local dist = (hrp.Position - dilarious.PrimaryPart.Position).Magnitude
            local hiding = liveChar:GetAttribute("Hiding")

            -- DEBUG live readout, only shown within 40 studs
            if dist < 40 then
                dbg(string.format("dist %.1f | range %s%s", dist, KILL_RANGE, hiding and " | HIDING" or ""))
            elseif not dbgPinned then
                dbgLabel.Visible = false
            end

            if hiding then return end -- matches Threat's own IgnoreHiding=false behavior
