...
task.spawn(function()
    while true do
        ...
        if not StopChecking and not CanMove and humanoid.MoveDirection.Magnitude > 0 then
            ...
            -- Same decrement every hit, fatal or not -- proven to stick already.
            -- On the fatal hit this is spelled out directly (matching Blackout's
            -- own technique word for word): this hit sets Health to 0, full stop.
            -- GuaranteeKill on top is reinforcement, not the primary mechanism.
            if wouldBeFatal then
                humanoid.Health = 0
