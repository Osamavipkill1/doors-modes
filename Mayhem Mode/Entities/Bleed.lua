...
if not okEvents then
    warn("[Mayhem/Bleed] Module_Events require failed -- flicker/shatter disabled.")
    ModuleEvents = {
...
if not okMain then
    warn("[Mayhem/Bleed] Main_Game require failed -- cam shake disabled.")
    MainGame = {
...
Spawner.runEntity = function(entityTable)
                ...
                if entityTable.Config.CanKill and not Char:GetAttribute("IsDead") and not Char:GetAttribute("Invincible") and not Char:GetAttribute("Hiding") and (getPlayerRoot().Position - entityModel.PrimaryPart.Position).Magnitude <= entityTable.Config.KillRange then
                    task.spawn(function()
                        ...
                        if #entityTable.Config.CustomDialog > 0 then
                            pcall(firesignal, ReSt.Bricks.DeathHint.OnClientEvent, entityTable.Config.CustomDialog, "Yellow")
                        end
    ...
    local function generateBlood()
        ...
        blood.Touched:Connect(function(hit)
            ...
            touchedHum.Health = math.max(touchedHum.Health - BLOOD_TOUCH_DAMAGE, 0)
