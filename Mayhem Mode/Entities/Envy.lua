...
if typeof(entityModel) == "Instance" and entityModel.ClassName == "Model" then
    ...
    if entityModel.PrimaryPart then
        ...
        -- FIX: damage now fires the INSTANT you stop moving (was a 1s wind-up), then
        -- repeats every 0.3s for as long as you stay stopped, and resets the instant
        -- you move again. Still not an outright one-hit kill -- you get repeated 10-dmg
        -- hits, not a single lethal blow, so there's a window to start moving again.
        task.spawn(function()
