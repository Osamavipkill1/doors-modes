...
-- independently with no messages sent at all. Generalized that same pattern to
-- every entity below: each gets its own JobId-seeded (but distinctly offset) set
-- of checkpoint rooms, and every client naturally arrives at those rooms at the
-- same time anyway, since Doors already syncs room progression for the party.
-- Stalker, Screech, and H-10 stay OUT of this and keep their own independent
-- per-client timers, unchanged.
local function BuildCheckpoints(seedOffset, count, minRoom, maxRoom)
    local r3 = game.JobId
    r3 = string.gsub(r3, '%D+', '')
    local seeded = Random.new(r3 + seedOffset)
    local rooms = {}
    for i = 1, count do
        rooms[i] = seeded:NextInteger(minRoom, maxRoom)
    end
    return rooms
end
...
-- 
task.spawn(function()
    -- quarter the range like before so the 4 checkpoints spread across the run
    local spawnvalues = {
        BuildCheckpoints(1, 1, 1, 25)[1],
        BuildCheckpoints(1, 1, 26, 50)[1],
        BuildCheckpoints(1, 1, 51, 75)[1],
        BuildCheckpoints(1, 1, 76, 101)[1],
    }
...
--
-- threat
local ThreatCheckpoints = BuildCheckpoints(2, 6, 1, 101)
...
task.spawn(function()
    game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
        if table.find(ThreatCheckpoints, game.ReplicatedStorage.GameData.LatestRoom.Value) then
            ThreatSpawn()
        end
    end)
end)
-- twister
local TwisterCheckpoints = BuildCheckpoints(3, 6, 1, 101)
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    if table.find(TwisterCheckpoints, game.ReplicatedStorage.GameData.LatestRoom.Value) then
...
--
-- mimic rush
local MimicRushCheckpoints = BuildCheckpoints(4, 6, 1, 101)
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
    if table.find(MimicRushCheckpoints, latestRoom) then
...
-- mimic ambush
local MimicAmbushCheckpoints = BuildCheckpoints(5, 3, 1, 101)
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
    if table.find(MimicAmbushCheckpoints, latestRoom) then
        if LastRoomEntity == nil and not SeekActive
            and latestRoom ~= 50 and latestRoom ~= 100
            and _G.Threat == "safe"
            and not game.Workspace:FindFirstChild("Mimic Rush")
...
-- obsession aka greed
local ObsessionCheckpoints = BuildCheckpoints(6, 8, 1, 101)
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
    if table.find(ObsessionCheckpoints, latestRoom) then
        if LastRoomEntity == nil and not SeekActive
...
-- envy
local EnvyCheckpoints = BuildCheckpoints(7, 4, 1, 101)
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
    if table.find(EnvyCheckpoints, latestRoom) then
        if LastRoomEntity == nil and not SeekActive
            and latestRoom ~= 50 and latestRoom ~= 100
...
--
-- manic eyes
local EyesCheckpoints = BuildCheckpoints(8, 8, 1, 101)
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
    if table.find(EyesCheckpoints, latestRoom) then
        if LastRoomEntity == nil and not SeekActive
            and not _G.GazeEntityActive
            and not game.Workspace:FindFirstChild("EyesMoving")
        then
...
-- bleed
local BleedCheckpoints = BuildCheckpoints(9, 5, 1, 101)
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
    if table.find(BleedCheckpoints, latestRoom) then
        if LastRoomEntity == nil and not SeekActive
            and latestRoom ~= 50 and latestRoom ~= 100
        then
            SetLast("Bleed", true)
...
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
    if table.find(DilariousCheckpoints, latestRoom) then
        if LastRoomEntity == nil and not SeekActive
            and latestRoom ~= 50 and latestRoom ~= 100
        then
