...
-- Player sync (best-effort): if 2+ players are running this script in the same
-- server, they agree on when the shared entities below spawn instead of each
-- rolling independently. Uses RegularVynixu's Communicator (built on
-- TextChatService) to elect a host -- lowest UserId among everyone currently
-- running a synced-capable copy -- and only the host makes spawn decisions,
-- broadcasting them to everyone else. Stalker, Screech, and H-10 stay OUT of this
-- on purpose and remain fully independent per player.
-- This depends on an external library and on TextChatService being usable in this
-- game. If Communicator fails to load, everything just falls back to fully
-- independent per-player spawning, same as before sync existed.
local Communicator = nil
local IsHost = true
pcall(function()
    Communicator = loadstring(game:HttpGet("https://raw.githubusercontent.com/RegularVynixu/Communicator/main/init.luau"))()
end)

local EntityURLs = {
    Threat = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Threat.lua",
    Twister = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Twister.lua",
    Blackout = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Blackout.lua",
    ["Mimic Rush"] = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/misc/pastebin1",
    ["Mimic Ambush"] = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/misc/pastebin2",
    Obsession = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Obsession.lua",
    Envy = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Envy.lua",
    ["Manic Eyes"] = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Manic%20Eyes.lua",
    ["Deer God"] = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/DeerGodV5.lua",
    Fog = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Fog.lua",
    Bleed = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Bleed.lua",
    Dilarious = "https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Dilarious.lua",
}

function SpawnNamedEntity(name)
...
if Communicator then
    pcall(function() Communicator.Config.ExcludeSelf = true end)

    task.spawn(function()
        while true do
            local ok, activeClients = pcall(function()
                return Communicator:Ping(1, true)
            end)
            if ok and activeClients and #activeClients > 0 then
                table.sort(activeClients, function(a, b) return a.UserId < b.UserId end)
                IsHost = (activeClients[1] == game.Players.LocalPlayer)
            else
                IsHost = true
            end
            task.wait(10)
    ...
    pcall(function()
        Communicator:Listen("MayhemEntitySpawn", function(sender, name, setLastName, extra)
            task.spawn(function()
                if setLastName then
                    pcall(function() SetLast(setLastName, extra) end)
                end
                SpawnNamedEntity(name)
            end)
        end)
    end)
end

-- Every spawn loop for a shared entity calls this instead of loading the entity
-- directly. If sync isn't available, or we ARE the host, this spawns locally (and
-- broadcasts to everyone else, if sync is available). If sync IS available and
-- we're NOT the host, this does nothing here -- the host's own call will broadcast
-- it and our Listen callback above will spawn it for us at that point.
function TriggerSyncedEntity(name, setLastName, extra)
    if Communicator and not IsHost then
        return
    end
    if setLastName then
        SetLast(setLastName, extra)
    end
    SpawnNamedEntity(name)
    if Communicator then
        pcall(function()
            Communicator:Send("MayhemEntitySpawn", name, setLastName, extra)
        end)
    end
end
...
local function BlackoutSpawn()
    ...
    then
        if not SeekActive then
            TriggerSyncedEntity("Blackout", "Blackout", true)
        else
...
local function ThreatSpawn()
    ...
    if hasWardrobe and not SeekActive and LastRoomEntity == nil
        ...
        and not game.Workspace:FindFirstChild("Mimic Ambush")
    then
        TriggerSyncedEntity("Threat", "Threat", false)
    else
...
task.spawn(function()
    while true do
        ...
        if LastRoomEntity == nil and not SeekActive then
            TriggerSyncedEntity("Twister", "Twister", true)
        end
    end
end)
--
...
task.spawn(function()
    while true do
        ...
        if LastRoomEntity == nil and not SeekActive
            ...
            and not game.Workspace:FindFirstChild("Mimic Ambush")
        then
            TriggerSyncedEntity("Mimic Rush", "Mimic Rush", true)
        end
    end
end)
-- mimic ambush
task.spawn(function()
    while true do
        ...
        if LastRoomEntity == nil and not SeekActive
            ...
            and not game.Workspace:FindFirstChild("Mimic Rush")
        then
            TriggerSyncedEntity("Mimic Ambush", "Mimic Ambush", true)
        end
    end
end)
--
...
task.spawn(function()
    while true do
        ...
        if LastRoomEntity == nil and not SeekActive
            ...
            and latestRoom ~= 50 and latestRoom ~= 100
        then
            TriggerSyncedEntity("Obsession", "Obs", true)
        end
    end
end)
--
...
task.spawn(function()
    while true do
        ...
        if LastRoomEntity == nil
            ...
            and latestRoom ~= 100
        then
            TriggerSyncedEntity("Envy", "Envy", true)
        end
    end
end)
...
task.spawn(function()
    while true do
        ...
        if LastRoomEntity == nil and not SeekActive
            ...
            and not game.Workspace:FindFirstChild("EyesMoving")
        then
            TriggerSyncedEntity("Manic Eyes", "Eyes", true)
        end
    end
end)
--
...
task.spawn(function()
    ...
    while not spawned1 or not spawned2 do
        ...
        if okToSpawn and not spawned1 and latestRoom >= window1 then
            spawned1 = true
            TriggerSyncedEntity("Deer God", "Deer God", true)
...
task.spawn(function()
    while true do
        task.wait(eb:NextInteger(150, 350))
        local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
        if LastRoomEntity == nil and not SeekActive
            and latestRoom ~= 50 and latestRoom ~= 100
        then
            TriggerSyncedEntity("Bleed", "Bleed", true)
        end
    end
end)
--
...
task.spawn(function()
    while true do
        task.wait(eb:NextInteger(150, 400))
        local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
        if LastRoomEntity == nil and not SeekActive
            and latestRoom ~= 50 and latestRoom ~= 100
        then
            TriggerSyncedEntity("Dilarious", "Dilarious", true)
        end
    end
end)
--
...
task.spawn(function()
    while true do
        ...
        if hum and hum.Health > 0 and LastRoomEntity == nil and not SeekActive then
            loadstring(game:HttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/A-90.lua"))()
        end
    end
end)
--]]
