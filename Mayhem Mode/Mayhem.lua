...
-- Shared death-cause helper, used by every custom entity so dying to them shows up
-- properly instead of "Unknown". We can't edit Doors' own Overview screen (that UI
-- isn't part of this mod, so we can't verify its exact structure from here), but we
-- CAN set the stats value it may read AND immediately caption "You died to X" using
-- the same Msg() mechanism the mod already relies on elsewhere. Installed via getgenv()
-- (matching Functions.lua's own approach) so every separately-loadstring'd entity file
-- can call it reliably regardless of loadstring environment boundaries.
getgenv().SetDeathCause = function(entityName)
    ...
    pcall(function()
        Msg("You died to " .. tostring(entityName) .. ".", 2)
    end)
end
...
function BlackoutSpawn()
    ...
    task.wait(1)
    -- FIX: also check for our OWN custom "Manic Rush"/"Manic Ambush" entities, not just
    -- the vanilla RushMoving/AmbushMoving/ThreatMoving markers (which never matched our mod's entities)
    if not game.Workspace:FindFirstChild("ThreatMoving")
        ...
        and not game.Workspace:FindFirstChild("Manic Rush")
        and not game.Workspace:FindFirstChild("Manic Ambush")
    then
...
function ThreatSpawn()
    while true do
        ...
        if hasWardrobe and not SeekActive and LastRoomEntity == nil then
...
task.spawn(function()
    while true do
        ...
        -- and Manic Eyes (or the official Doors Eyes) can never be active together
