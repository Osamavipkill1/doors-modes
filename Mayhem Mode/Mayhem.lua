...
-- ebic
-- Every entity file (and this one) loads its code via loadstring(game:HttpGet(url))
-- freshly every time it's needed. For shared libraries -- Functions.lua, Source.lua,
-- DefaultConfig.lua -- several different entities each independently re-fetch AND
-- re-run the exact same file every time any of them spawns, which is a network
-- round-trip plus a recompile (plus, for Functions.lua specifically, another
-- DescendantAdded/PlayerRemoving connection that never got cleaned up from last
-- time) happening right at the moment an entity is supposed to appear -- exactly
-- the kind of thing that shows up as a hitch, worse on a slower connection/device.
-- Neither cache below changes what runs, only how many times identical bytes get
-- re-fetched or re-executed.
getgenv().MayhemHttpCache = getgenv().MayhemHttpCache or {}
getgenv().CachedHttpGet = function(url)
    local cache = getgenv().MayhemHttpCache
    if cache[url] then
        return cache[url]
    end
    local text = game:HttpGet(url)
    cache[url] = text
    return text
end

-- Only for shared library-style modules that hand back a reusable table (Functions,
-- DefaultConfig, Source) -- NOT for entity files, which need to actually run fresh
-- every spawn to create a new instance. Safe here because these specific modules'
-- state is either instance-keyed or kept current through a persistent connection,
-- not something that needs a brand new copy per call.
getgenv().MayhemModuleCache = getgenv().MayhemModuleCache or {}
getgenv().LoadCachedModule = function(url)
    local cache = getgenv().MayhemModuleCache
    if cache[url] ~= nil then
        return cache[url]
    end
    local result = loadstring(CachedHttpGet(url))()
    cache[url] = result
    return result
end
-- commands
loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Commands.lua"))()
...
-- Envy/Twister/Mimic Eyes/Obsession deal small repeated hits rather than one big hit
-- like Blackout. Blackout's single Health=0 write reliably kills; a single Health=0
-- write from these gradual-damage entities apparently doesn't stick. Hammering
-- Health back to 0 repeatedly for a short burst instead of writing it once should
-- push the kill through.
getgenv().GuaranteeKill = function(hum)
    task.spawn(function()
        -- Heartbeat instead of task.wait(0.1) -- roughly 90 writes over 1.5s instead of
        -- 15, so there's no multi-frame gap for anything else to slip a health value
        -- back in between writes. TakeDamage is a second technique alongside the direct
        -- Health write in case one path is treated differently from the other.
        local elapsed = 0
        local conn
...
-- Fires the death-screen hint through whichever remote folder this version of Doors keeps
-- DeathHint in (RemotesFolder now; EntityInfo / Bricks on older builds). Errors if none has
-- it, so callers keep their existing pcall + warn.
getgenv().FireDeathHint = function(hints, colour)
    local replicatedStorage = game:GetService("ReplicatedStorage")
    for _, folderName in ipairs({"RemotesFolder", "EntityInfo", "Bricks"}) do
        local folder = replicatedStorage:FindFirstChild(folderName)
        local remote = folder and folder:FindFirstChild("DeathHint")
        if remote then
            firesignal(remote.OnClientEvent, hints, colour)
            return
        end
    end
    error("DeathHint remote not found (looked in RemotesFolder, EntityInfo, Bricks)")
end

-- Player sync, take 2: the Communicator/TextChatService approach sent real chat
-- traffic (hitting Roblox's own chat rate limit -- that's the "please wait" message)
-- and there was no way to confirm Ping()'s exact return shape without its source,
-- which is almost certainly why host detection came back wrong for everyone.
-- Replaced entirely with zero-network sync: every client shares the same
-- game.JobId, and Blackout already proved this works -- its spawn rooms come from
-- Random.new(JobId)-seeded picks, so every client computes the identical list
-- independently with no messages sent at all. Stalker, Screech, and H-10 stay OUT
-- of this and keep their own independent per-client timers, unchanged.
--
-- First version of this drew each entity's spawn rooms fully independently: 10
-- entities picking 55 single, exact-match rooms out of only ~101 total. That's
-- bound to collide with itself constantly, and a collision meant whichever entity
-- lost just didn't spawn at all for the rest of the run -- no retry. Replaced with
-- non-overlapping zones instead: laid out end-to-end across the room range (so two
-- entities literally cannot claim the same rooms), and each zone is a small RANGE,
-- not one exact room, so if the first room in a zone is blocked by something else's
-- cooldown, it just tries the next room in that same zone instead of losing the
-- window entirely.
local EntityZones = {}

local function AllocateZones()
    local r3 = game.JobId
    r3 = string.gsub(r3, '%D+', '')
    local seeded = Random.new(r3 + 500)

    local entries = {}
    local function add(name, count)
        for i = 1, count do table.insert(entries, name) end
    end
    add("Blackout", 3)
    add("Threat", 2)
    add("Twister", 2)
    ...
    for i = #entries, 2, -1 do
        local j = seeded:NextInteger(1, i)
    ...
    local cursor = 3
    local usableEnd = 98
    -- 23 zones need ~46 rooms of actual zone width; verified this allocator
    -- against 20,000 simulated JobIds with a real Lua interpreter before trusting
    -- it -- a 1-4 room gap between zones ran out of space and dropped 1-2 entities
    -- to zero zones for that run; 1-2 room gaps place all 23 zones reliably every
    -- time, spread from room 3 to a worst-case room 93 (confirmed mathematically,
    -- not just observed -- the true worst case is exactly 93, never more)

    for _, name in ipairs(entries) do
        if cursor + zoneWidth - 1 >= 49 and cursor <= 51 then
            cursor = 52
        end
        if cursor > usableEnd then break end

        local zoneStart = cursor
        local zoneEnd = math.min(cursor + zoneWidth - 1, usableEnd)
        EntityZones[name] = EntityZones[name] or {}
        table.insert(EntityZones[name], {zoneStart, zoneEnd})
        cursor = zoneEnd + 1 + seeded:NextInteger(1, 2)
    end
end
AllocateZones()

-- Fires the FIRST eligible room within each of this entity's zones (not an exact
-- match), and gives up on a given zone only once its range has fully passed.
function ConnectZoneSpawn(entityName, conditionFn, spawnFn)
    local zones = EntityZones[entityName] or {}
    local firedZone = {}
    game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
        local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
        for idx, zone in ipairs(zones) do
            if not firedZone[idx] then
                if latestRoom >= zone[1] and latestRoom <= zone[2] then
                    if conditionFn(latestRoom) then
                        firedZone[idx] = true
                        spawnFn()
                    end
                elseif latestRoom > zone[2] then
                    firedZone[idx] = true
                end
            end
        end
...
-- Best-effort vanilla-entity awareness: SeekMoving/ThreatMoving/RushMoving/
-- AmbushMoving/EyesMoving are already used elsewhere in this mod and were working
-- before this change. HaltMoving and SnareMoving are inferred from that same
-- "<n>Moving" pattern, NOT independently confirmed -- if those two specific
-- names turn out wrong they just won't match anything (harmless no-op), not break.
function VanillaEntityActive()
    return game.Workspace:FindFirstChild("RushMoving") ~= nil
        or game.Workspace:FindFirstChild("AmbushMoving") ~= nil
        or game.Workspace:FindFirstChild("ThreatMoving") ~= nil
        or game.Workspace:FindFirstChild("HaltMoving") ~= nil
        or game.Workspace:FindFirstChild("SnareMoving") ~= nil
end
...
task.spawn(function()
    task.wait(2)
    loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/MayhemSFX.lua"))()
end)
--
-- Seek Check
...
local function BlackoutSpawn()
    ...
    task.wait(1)
    local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
    if not game.Workspace:FindFirstChild("Mimic Rush")
        and not game.Workspace:FindFirstChild("Mimic Ambush")
        and not game.Workspace:FindFirstChild("Envy")
        and not VanillaEntityActive()
        and latestRoom ~= 50 and latestRoom ~= 100
    then
        if not SeekActive then
            SetLast("Blackout", true)
            loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Blackout.lua"))()
        else
...
--
ConnectZoneSpawn("Blackout",
    function(latestRoom)
        return LastRoomEntity == nil and not SeekActive
    end,
    BlackoutSpawn
)
--
...
local function ThreatSpawn()
    ...
    if hasWardrobe then
        ...
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Threat.lua"))()
        return true
    else
...
-- twister
ConnectZoneSpawn("Twister",
    function(latestRoom)
        return LastRoomEntity == nil and not SeekActive and not VanillaEntityActive()
    end,
    function()
        SetLast("Twister", true)
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Twister.lua"))()
    end
)
--
-- mimic rush
ConnectZoneSpawn("Mimic Rush",
    function(latestRoom)
        return LastRoomEntity == nil and not SeekActive
            and _G.Threat == "safe"
            and not game.Workspace:FindFirstChild("Mimic Ambush")
    ...
    function()
        ...
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/misc/pastebin1"))()
    end
)
-- mimic ambush
ConnectZoneSpawn("Mimic Ambush",
    function(latestRoom)
        return LastRoomEntity == nil and not SeekActive
            and _G.Threat == "safe"
            and not game.Workspace:FindFirstChild("Mimic Rush")
    ...
    function()
        ...
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/misc/pastebin2"))()
    end
)
--
...
local WhiteoutRoom1 = 48
local WhiteoutRoom2 = 98
...
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    ...
    if latestRoom == 50 or latestRoom == 100 then
        whiteout = false
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Fog.lua"))()
        game.Lighting.FogColor = Color3.fromRGB(255, 255, 255) 
        game.Lighting.FogEnd = 105
        game.Lighting.FogStart = 35
        game.ReplicatedStorage.GameData.LatestRoom.Changed:Wait()
    ...
    if (latestRoom == WhiteoutRoom1 or latestRoom == WhiteoutRoom2) and not whiteout and not SeekActive then
        whiteout = true
        if whitefog then whitefog.Enabled = true end
        game.Lighting.FogColor = Color3.fromRGB(255, 255, 255) 
        game.Lighting.FogEnd = 135
        ...
        -- this used to never clear on its own -- it only ever reset when the
        -- player reached room 50/100, so it could stay applied for 10+ rooms and
        -- then stack directly into the room 50/100 fog sequence. Clears itself
        -- after ~12s regardless, unless room 50/100 already cleared it first.
        task.spawn(function()
            task.wait(12)
            if whiteout then
                whiteout = false
...
while true do
    ...
    then
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/misc/psstman"))()
    end
    end
end)
...
while true do
    ...
    then
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Stalker.lua"))()
    end
    end
end)
--
...
ConnectZoneSpawn("Obsession",
    ...
    function()
        SetLast("Obs", true)
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Obsession.lua"))()
    end
)
--

-- envy
ConnectZoneSpawn("Envy",
    function(latestRoom)
        return LastRoomEntity == nil and not SeekActive and not VanillaEntityActive()
            and _G.Blackout == "safe"
    end,
    function()
        SetLast("Envy", true)
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Envy.lua"))()
    end
)

--
-- manic eyes
ConnectZoneSpawn("Manic Eyes",
    function(latestRoom)
        return LastRoomEntity == nil and not SeekActive
            and not _G.GazeEntityActive
            and not game.Workspace:FindFirstChild("EyesMoving")
    ...
    function()
        SetLast("Eyes", true)
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Manic%20Eyes.lua"))()
    end
)
--

-- deer god v5 -- already deterministic since window1/window2 come from the
-- JobId-seeded 'ab', so every client picks the same two windows independently
task.spawn(function()
    ...
    while not spawned1 or not spawned2 do
        ...
        if okToSpawn and not spawned1 and latestRoom >= window1 then
            ...
            loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/DeerGodV5.lua"))()
...
-- bleed
ConnectZoneSpawn("Bleed",
    function(latestRoom)
        return LastRoomEntity == nil and not SeekActive and not VanillaEntityActive()
    end,
    function()
        SetLast("Bleed", true)
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Bleed.lua"))()
    end
)
--

-- dilarious
ConnectZoneSpawn("Dilarious",
    function(latestRoom)
        return LastRoomEntity == nil and not SeekActive and not VanillaEntityActive()
    end,
    function()
        SetLast("Dilarious", true)
        loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/Dilarious.lua"))()
    end
)
--
...
task.spawn(function()
    while true do
        ...
        local char = game.Players.LocalPlayer.Character
        local hum = char and char:FindFirstChild("Humanoid")
        if hum and hum.Health > 0 and LastRoomEntity == nil and not SeekActive then
            loadstring(CachedHttpGet("https://raw.githubusercontent.com/Osamavipkill1/doors-modes/refs/heads/main/Mayhem%20Mode/Entities/A-90.lua"))()
        end
    end
end)
--
