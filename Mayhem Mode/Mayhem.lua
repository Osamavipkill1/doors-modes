...
-- Shared death-cause helper, used by every custom entity so dying to them is reported
-- correctly. Installed via getgenv() (matching Functions.lua's own approach) so every
-- separately-loadstring'd entity file can call it reliably regardless of loadstring
-- environment boundaries.
-- NOTE: the on-screen "You died to X" Msg() caption has been removed per request --
-- this now only sets the stats value. Whether Doors' own Overview screen renders a
-- non-vanilla name properly or falls back to "Unknown" is up to that screen's own
-- internal logic, which lives outside this mod and isn't something this can verify
-- or patch from here.
...
-- FIX: preload the Stalker and Blackout jumpscare images as early as possible --
-- right at script start, giving them the ENTIRE session's runtime to finish caching
-- instead of only the (now much shorter, with a 1s Stalker look-time) window between
-- an entity spawning and its jumpscare actually firing.
task.spawn(function()
    pcall(function()
        local preloadImgs = {}
...
game.ReplicatedStorage.GameData.LatestRoom.Changed:Connect(function()
    for _, v in next, game.Workspace.CurrentRooms[game.ReplicatedStorage.GameData.LatestRoom.Value].Assets:GetDescendants() do
        ...
        if string.match(v.Name, "Painting") and not string.match(v.Name, "Seek") and v.Name ~= "Paintings" then
            ...
            -- FIX: these are TEXTURE ids (not asset/catalog ids) — asset ids show as solid white
...
function BlackoutSpawn()
    ...
    -- FIX: renamed Manic Rush/Ambush -> Mimic Rush/Ambush to match the new display
    -- names, and added the room 50/100 exclusion
...
task.spawn(function()
    ...
    -- FIX: 50 and 100 could already land in these ranges (26-50, 76-101) -- strip
    -- them out so Blackout can never be picked for either room
...
function ThreatSpawn()
    while true do
        ...
        -- FIX: search recursively (GetDescendants) instead of a single FindFirstChild,
        -- since "Wardrobe" may be nested and the exact naming can't be verified from here.
        -- Wrapped in pcall so a room with an unusual Assets structure can't kill this loop.
        ...
        local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
        -- FIX: added room 50/100 exclusion
...
while true do
    ...
    latestRoom ~= 50 and latestRoom ~= 100
    then
...
while true do
    ...
    local char = game.Players.LocalPlayer.Character
    local hum = char and char:FindFirstChild("Humanoid")
    -- FIX: added _G.StalkerActive so a second copy can't spawn mid-encounter
    if _G.Blackout == "safe" and
    _G.Threat == "safe" and
    hum and hum.Health > 0 and
    not SeekActive and
...
task.spawn(function()
    while true do
        ...
        local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
        -- FIX: added _G.GazeEntityActive + vanilla "EyesMoving" check so Obsession
        -- and Mimic Eyes (or the official Doors Eyes) can never be active together,
        -- plus the room 50/100 exclusion
        if LastRoomEntity == nil and not SeekActive
...
-- april fools!!! (envy)
-- FIX: Envy.lua used to register a PERMANENT LatestRoom.Changed connection on every
-- invocation, so after a while there were many stacked connections all spawning a new
-- Envy on every single room change -- that's why it felt like "every room". Envy.lua
-- itself is now a one-shot script (spawn once, wait one room change, despawn, done),
-- so this periodic loop is the ONLY thing controlling how often it spawns.
-- Interval roughly doubled vs Twister's 60-280s (half the spawn chance), plus the
-- Seek/room50/room100 exclusion you asked for, plus the standard LastRoomEntity gate
-- so it doesn't pile on top of another entity.
task.spawn(function()
    while true do
...
task.spawn(function()
    while true do
        ...
        -- FIX: added _G.GazeEntityActive + vanilla "EyesMoving" check (see Obsession above)
        if LastRoomEntity == nil and not SeekActive
            and not _G.GazeEntityActive
            and not game.Workspace:FindFirstChild("EyesMoving")
        then
...
-- deer god v5 -- doesn't have its own spawn-rate logic (it's a straight
-- create-and-run script, same shape as the other one-shot entity files), so the
-- "once a run, really rare" behavior lives entirely here: pick ONE random room
-- somewhere across a typical run, and the very first time the player reaches or
-- passes it, spawn it exactly once and never again for the rest of the session.
task.spawn(function()
    local deerGodRoom = ab:NextInteger(10, 90)
    local deerGodSpawned = false
    while not deerGodSpawned do
        game.ReplicatedStorage.GameData.LatestRoom.Changed:Wait()
        local latestRoom = game.ReplicatedStorage.GameData.LatestRoom.Value
        if latestRoom >= deerGodRoom
            and not SeekActive
            and latestRoom ~= 50 and latestRoom ~= 100
